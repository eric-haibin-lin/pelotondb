...
bool BWTree<KeyType, ValueType, KeyComparator>::InsertEntry(
  ...
  bool complete = false;
  // just call InsertEntry on root. retry until success
  while (!complete) {
...
BWTree<KeyType, ValueType, KeyComparator>::ScanAllKeys() {
  ...
  // iteratively call scan from the root of BWTree
  auto curr_epoch = epoch_manager_.GetCurrentEpoch();
  LPID next_page = root_;
  do {
...
void BWTree<KeyType, ValueType, KeyComparator>::CompressAllPages() {
  // assume there's no delta for merges for the moment
  ...
  // compress all delta chains in the mapping table
...
int BWTree<KeyType, ValueType, KeyComparator>::GetChild(
  ...
  // ignore the last entry in the IPage while searching
...
LPID BWTree<KeyType, ValueType, KeyComparator>::ScanHelper(
  ...
  if (index_key != nullptr) {
    // special case: scan on the matched child directly
...
LPID BWTree<KeyType, ValueType, KeyComparator>::ScanHelper(
  ...
  if (index_key != nullptr) {
    ...
    for (; index < (long)size; index++) {
      ...
      // check if expression constraints are met
...
bool BWTree<KeyType, ValueType, KeyComparator>::MatchLeadingColumn(
  ...
  // find the position of the leading column
...
bool IPage<KeyType, ValueType, KeyComparator>::InsertEntry(KeyType key,
  ...
  // check if split should be performed first
  if (should_split_) {
    SplitNodes(self);
    return false;
  }
...
bool IPage<KeyType, ValueType, KeyComparator>::InstallParentDelta(
  ...
  // search for the correct position to install
...
void IPage<KeyType, ValueType, KeyComparator>::SplitNodes(LPID self) {
  ...
  KeyType maxLeftSplitNodeKey = children_[size_ / 2].first;
  int modifiedIndex = size_ / 2;
  LPID newIpageLPID = this->map->GetMappingTable()->InstallPage(newIpage);
  ...
  bool swapSuccess =
      this->map->GetMappingTable()->SwapNode(self, this, splitDelta);
  ...
  LPID new_root_LPID = this->map->GetMappingTable()->InstallPage(new_root);
  new_root->size_ = 2;
  ...
  bool successFlag = __sync_bool_compare_and_swap(
      this->map->GetRootLPIDAddress(), self, new_root_LPID);
...
LPID LPageSplitDelta<KeyType, ValueType, KeyComparator>::ScanKey(
  ...
  // see if request key is greater than separator key
...
LPageSplitDelta<KeyType, ValueType, KeyComparator>::BuildNodeState(int) {
  ...
  LNodeStateBuilder<KeyType, ValueType, KeyComparator> *lbuilder =
      reinterpret_cast<LNodeStateBuilder<KeyType, ValueType, KeyComparator> *>(
          builder);
  // update right sibling of builder
...
bool LPageSplitDelta<KeyType, ValueType, KeyComparator>::InsertEntry(
  ...
  // wait until this split is completed to avoid premature compression
  if (!this->split_completed_) {
    LOG_INFO("Returning early because split in progress");
    return false;
  }

  // this key is greater than modified_key_, go to right split node
...
bool LPageSplitDelta<KeyType, ValueType, KeyComparator>::DeleteEntry(
  ...
  // wait until split completes to avoid premature comression
  if (!this->split_completed_) {
    LOG_INFO("Returning early because split in progress");
    return false;
  }
  ...
  // now install delta below itself
  ...
  // since same keys can span across multiple Lpages, cascade deletes to all
  // right siblings with the same right most key
...
bool LPageUpdateDelta<KeyType, ValueType, KeyComparator>::InsertEntry(
  ...
  // wait for split to complete, avoid premature compression of chain
  if (should_split_) {
  ...
  LPageUpdateDelta<KeyType, ValueType, KeyComparator> *new_delta =
      new LPageUpdateDelta<KeyType, ValueType, KeyComparator>(
          ...
          this->right_sibling);
  // perform delta insert, and compress if required
  bool status = this->PerformDeltaInsert(my_lpid, new_delta);
  if (!status) {
    delete new_delta;
  }
...
bool LPageUpdateDelta<KeyType, ValueType, KeyComparator>::DeleteEntry(
  ...
  // perform delta insert, compress if required
  bool status = this->PerformDeltaInsert(my_lpid, new_delta);
  if (!status) {
    delete new_delta;
    return false;
  }
  // cascade delete, because same key may span across multiple pages
  if (this->right_sibling != INVALID_LPID &&
...
bool LPage<KeyType, ValueType, KeyComparator>::InsertEntry(KeyType key,
                                                           ValueType location,
                                                           LPID self) {
  // try splitting this LPage if possible
  ...
  // install delete delta above itself
  bool status = this->map->GetMappingTable()->SwapNode(self, this, new_delta);
...
bool LPage<KeyType, ValueType, KeyComparator>::DeleteEntry(KeyType key,
  ...
  // see if this node can be split
  bool should_split = this->size_ > LPAGE_SPLIT_THRESHOLD;
  if (should_split) {
    if (update_parent) {  // try splitting only if update_parent set to true
      this->SplitNodes(self);
      return false;
    }
  }
  ...
  // since keys can span across multiple pages, cascade the delete to all pages
...
bool LPage<KeyType, ValueType, KeyComparator>::SplitNodes(LPID self) {
  ...
  if (swapSuccess == false) {
    ...
    // doing the atomic half split. Now if we try and install our own Insert /
    // Delete /
    // Update delta, it will automatically be created below the LPageSplitDelta
    return false;
  }
  ...
  // this thread won in the half split.

  // optimization to avoid posting delta on parent if split key and right most
  // key are same
  ...
  // keep trying to install update delta on parent until we succeed
  do {
    IPageUpdateDelta<KeyType, ValueType, KeyComparator> *new_delta =
        new IPageUpdateDelta<KeyType, ValueType, KeyComparator>(
            this->map, nullptr, maxLeftSplitNodeKey, this->GetRightMostKey(),
  ...
  // allow the split delta to start processing requests
  splitDelta->SetSplitCompleted();
